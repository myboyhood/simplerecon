    ...
    with torch.inference_mode():
        ...
        for scan in tqdm(scans):
            ...
            dataset = dataset_class(
                        ...
                        include_full_res_depth=False, # before is True, wzy change to False
                        ...
                        include_full_depth_K=False, # before is True, wzy change to False
            ...
            # wzy change depth usage to False
            use_depth = False
            ...
            for batch_ind, batch in enumerate(tqdm(dataloader)):
                ...
                # wzy manually set valid mask
                batch_size, _, height, width = upsampled_depth_pred_b1hw.shape
                valid_mask_b = torch.ones((batch_size, 1, height, width), dtype=torch.bool)
                # print(f"valid_mask_b: {valid_mask_b}")

                # Check if there any valid gt points in this sample
                if use_depth and (valid_mask_b).any():
                    print(f"into compute metrics")
                    # compute metrics
                ...
                ######################### DEPTH FUSION #########################
                if opts.run_fusion: # wzy add use_depth condition
